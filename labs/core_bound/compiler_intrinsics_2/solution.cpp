...
unsigned solution(const std::string &inputContents) {
  ...
  if (strLength >= VECTORIZATION_FACTOR) {  // size of YMM vector
    const __m256i eol = _mm256_set1_epi8('\n');
    auto *buffer = inputContents.data();
    uint32_t curLineBegin = 0;
    for (; pos + VECTORIZATION_FACTOR < strLength;
         pos += VECTORIZATION_FACTOR) {
      ...
      while (mask) {
        ...
        if (pos < curLineBegin) curLineLength = maskPos;
...
//uint32_t solution1(const std::string &inputContents) {
